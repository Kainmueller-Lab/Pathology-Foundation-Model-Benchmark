...
class TestSplitToTiles(unittest.TestCase):
    ...
    def test_center_pad_to_size(self):
        ...
        self.assertEqual(
            padded_array_2d_int.shape, desired_shape_2d, "2D array should be padded to desired shape with int input."
        )
        self.assertTrue(
        ...
        self.assertEqual(
            padded_array_3d_int.shape, desired_shape_3d, "3D array should be padded to desired shape with int input."
        )
        self.assertTrue(
    ...
    def test_transform_to_tiles(self):
        ...
        if semantic_mask_tiles is not None:
            self.assertEqual(
                len(semantic_mask_tiles), num_tiles, "semantic_mask_tiles should match image_tiles in length."
            )
        if instance_mask_tiles is not None:
            self.assertEqual(
                len(instance_mask_tiles), num_tiles, "instance_mask_tiles should match image_tiles in length."
            )
        ...
        for idx, img_tile in enumerate(image_tiles):
            ...
            self.assertEqual(img_tile.shape, (3, 224, 224), "Image tile should be of shape (3, 224, 224).")

            # Check semantic mask tile if available
            if semantic_mask_tiles is not None:
                semantic_mask_tile = semantic_mask_tiles[idx]
                self.assertIsInstance(
                    semantic_mask_tile, np.ndarray, "Each semantic mask tile should be a NumPy array."
                )
                self.assertEqual(
                    semantic_mask_tile.shape, (224, 224), "Semantic mask tile should be of shape (224, 224)."
                )
            ...
            if instance_mask_tiles is not None:
                ...
                self.assertIsInstance(
                    instance_mask_tile, np.ndarray, "Each instance mask tile should be a NumPy array."
                )
                self.assertEqual(
                    instance_mask_tile.shape, (224, 224), "Instance mask tile should be of shape (224, 224)."
                )
    ...
    def test_transform_to_tiles_edge_cases(self):
        ...
        tile = tiles[0]
        self.assertEqual(tile.shape, (3, 500, 500), "Tile should match image dimensions without padding.")
